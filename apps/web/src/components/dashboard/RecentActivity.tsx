import { transactions } from '../../lib/demo-data';
import { Card } from './Card';

export function RecentActivity() {
  return (
    <Card id="transactions" className="overflow-hidden">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-6">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">
            Activity preview
          </h2>
          <p className="mt-1 text-[11px] font-medium text-slate-600">
            Synthetic records · no source connections
          </p>
        </div>
        <span className="rounded-full bg-mist-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600">
          Preview
        </span>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full min-w-[660px] border-collapse text-left">
          <caption className="sr-only">
            Four synthetic example financial records
          </caption>
          <thead>
            <tr className="border-b border-slate-100 text-[10px] font-semibold tracking-[0.1em] text-slate-600 uppercase">
              <th scope="col" className="px-5 py-3 sm:px-6">
                Merchant or person
              </th>
              <th scope="col" className="px-4 py-3">
                Category
              </th>
              <th scope="col" className="px-4 py-3">
                Source
              </th>
              <th scope="col" className="px-4 py-3 text-right">
                Amount
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {transactions.map((transaction) => (
              <tr key={transaction.id} className="group hover:bg-mist-50/70">
                <th scope="row" className="px-5 py-3.5 font-normal sm:px-6">
                  <div className="flex items-center gap-3">
                    <span
                      className={`grid size-9 shrink-0 place-items-center rounded-xl text-[10px] font-bold ${transaction.merchantTone}`}
                      aria-hidden="true"
                    >
                      {transaction.merchantInitials}
                    </span>
                    <span>
                      <span className="block text-xs font-semibold text-slate-700">
                        {transaction.merchant}
                      </span>
                      <span className="mt-0.5 block text-[10px] font-medium text-slate-600">
                        {transaction.date}
                      </span>
                    </span>
                  </div>
                </th>
                <td className="px-4 py-3.5 text-xs text-slate-500">
                  {transaction.category}
                </td>
                <td className="px-4 py-3.5">
                  <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-medium text-slate-500">
                    {transaction.source}
                  </span>
                </td>
                <td
                  className={`px-4 py-3.5 text-right text-xs font-semibold tabular-nums ${
                    transaction.direction === 'in'
                      ? 'text-leaf-700'
                      : 'text-slate-800'
                  }`}
                >
                  {transaction.amount}
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </Card>
  );
}
